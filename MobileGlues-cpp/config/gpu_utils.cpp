...
int hasVulkan12() {
    ...
    appInfo.pEngineName = "VinzzRenderer";
...
// VinzzRenderer additions - Adreno 650 (Snapdragon 870) optimizations
int isAdreno650(const char* gpu) {
    if (!gpu) return 0;
    return isAdreno(gpu) && (strstr(gpu, "650") != nullptr);
}

int isAdreno6xx(const char* gpu) {
    if (!gpu) return 0;
    if (!isAdreno(gpu)) return 0;
    const char* series[] = {"61","62","63","64","65","66","68","69", nullptr};
    for (int i = 0; series[i]; i++) {
        if (strstr(gpu, series[i])) return 1;
    }
    return 0;
}
...
int checkASTCSupport() {
    ...
    if (!glesLib) return 0;
    auto glGetString = (const GLubyte* (*)(GLenum))dlsym(glesLib, "glGetString");
    if (!glGetString) { dlclose(glesLib); return 0; }
    const char* exts = reinterpret_cast<const char*>(glGetString(GL_EXTENSIONS));
    int result = exts && (strstr(exts, "GL_KHR_texture_compression_astc_ldr") != nullptr);
    dlclose(glesLib);
    return result;
}

int getOptimalMultidrawForAdreno650() {
    return 3; // PreferMultidrawIndirect
}
